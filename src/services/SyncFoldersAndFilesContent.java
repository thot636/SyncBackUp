...
public class SyncFoldersAndFilesContent {

	private List <Path>listOfOutstandingItems;

	public boolean compareIdentityOption1(Path source, Path target){
		boolean bool=false;
		try{
		FileUtils.contentEquals(source.toFile(), target.toFile());
		}catch (IOException e){
	...
	public boolean compareIdentityOption2(Path source, Path target){
		boolean bool=false;
	...
	SyncFoldersAndFilesContent(Path source, Path target){
		try {
	...
	class CopyFoldersAndFiles extends SimpleFileVisitor<Path>{
		...
		public FileVisitResult visitFile(Path file, BasicFileAttributes attrs) {
			...
			if (!Files.exists(newFile) || !compareIdentityOption2(file, newFile) )
			{
				try {
				...
				// prevent to copy files which are exist in source folder ( it is set tu true 
				// in preVisitDirectory when new folder is found
	...
	public static void main(String [] args){
			...
			Path dest = Paths.get("C:\\HYW_DataCopy");
			//EnumSet<FileVisitOption> opts = EnumSet.of(FileVisitOption.FOLLOW_LINKS);
			SyncFoldersAndFilesContent ca = new SyncFoldersAndFilesContent(source, dest);
			//Files.walkFileTree(source, opts, Integer.MAX_VALUE, ca);
			//System.out.println(ca.testing());
	...
	public boolean testing(){
		...
		FileUtils.contentEquals(source.toFile(), dest.toFile());
		}catch (IOException e){
